...
// Importamos Login (Asegúrate de copiar este componente de tu proyecto anterior a ./components/Login.tsx)
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        // Ajusta esta condición según tus reglas de negocio
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, "matches"), (snapshot) => {
      ...
      // Lógica de limpieza automática (Tomada de App_anterior)
      const now = new Date();
  ...
  const handleUpdateUser = async (updatedUser: User) => {
    ...
    await updateDoc(userRef, {
        ...
        dob: updatedUser.dob || '',
        // No actualizamos email/avatar aquí generalmente si vienen de Auth provider, 
        // pero puedes agregarlo si tu formulario lo permite.
    });
    ...
    // Nota: No necesitamos actualizar 'matches' manualmente, el listener onSnapshot lo hará
    // cuando detecte cambios si tus documentos de matches guardan referencias o copias de usuarios.
    // Si guardas copias del usuario dentro del array 'players' en matches, 
    // necesitarías un Cloud Function o un Batch Update complejo.
    // Por simplicidad, asumimos que la UI se refresca.
  };
  ...
  const handleCreateMatch = async (date: string, time: string, court: string) => {
    ...
    try {
        ...
        // Construimos el objeto Match local para el modal de compartir
  ...
  const handleJoinMatch = async (matchId: string) => {
    ...
    // Verificar si se completaron los 4 para crear equipos automáticamente (Lógica App_anterior)
  ...
  // Si no está autenticado, mostramos el Login (Traído de App_anterior)
  if (!isAuthenticated) {
      return <Login />;
  }

  // Filtramos partidos para la vista "Mis Partidos" vs "Todos"
  // Pero usamos 'matches' que viene directo de Firebase
  // Podrías usar useMemo aquí si la lista es muy grande, pero para <100 partidos está bien así.
    ...
    <div className="min-h-screen bg-slate-950 font-sans relative selection:bg-orange-500 selection:text-white">
      ...
      <div className="relative z-10">
        <Header
            ...
            userAvatar={currentUser?.avatar} // Pasar avatar al header si lo soporta
        />
